...
class Bank:
    ...
    def check_total_loan_amount(self):
        total_loan_amount = sum(user.total_loan for user in self.users)
        print('Total Loan Amount : ', total_loan_amount)
...
class User(Person):
    def __init__(self, name, email, address, account_type):
        ...
        if account_type in ['Savings', 'Current']:
            ...
            raise ValueError("Invalid account type. Please choose from 'Savings' or 'Current'.")


    def create_account(self, bank):
        bank.add_user(self)
        print(f"User account for {self.name} created successfully.")
...
def admin_menu(admin, bank):
    while True:
        ...
        if admin_choice == '1':
            user_name = input("Enter The Name Of The User To Delete : ")
            user_to_delete = next((u for u in bank.users if u.name == user_name), None)
            if user_to_delete:
                admin.delete_user_account(user_to_delete, bank)
            else:
                print("User not found.")
